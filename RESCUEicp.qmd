...
```{r wrangle data}
# Define your models and outcomes
models <- list(
  "Cumulative" = cumulative,
  "Unequal Variance" = unequal_variance,
...
# Create a tidy dataframe using purrr::map_dfr
library(tidyverse)

results_data <- map_dfr(names(models), function(model_name) {
  map_dfr(names(outcomes), function(outcome_name) {
    result <- compare_groups(models[[model_name]], "Medical", "Surgical", outcomes[[outcome_name]])
    
    tibble(
      model = model_name,
      outcome = outcome_name,
      risk_reduction = 1 - result$mean,  # Convert relative risk to risk reduction
      lower_95 = 1 - result$pi["97.5%"],  # Note: reversed because we're subtracting from 1
      upper_95 = 1 - result$pi["2.5%"],   # Note: reversed because we're subtracting from 1
      prob_benefit = result$prob_less_than_1
    )
  })
})
...
```{r}
final_plot <- results_data %>%
  mutate(
    # Ensure consistent factor ordering
    outcome = factor(outcome, levels = c("Death/Vegetative", "Poor Outcome")),
    model = factor(model, levels = c("Cumulative", "Unequal Variance", "Category-Specific"))
  ) %>%
  ggplot(aes(x = model, y = risk_reduction)) +
  ...
  geom_pointrange(aes(ymin = lower_95, ymax = upper_95),
                  ...
                  linewidth = 0.8,
                  color = "black") +
  # Separate panels for each outcome
  facet_wrap(~outcome, nrow = 1, scales = "free_y") +
  scale_y_continuous(labels = scales::percent_format(accuracy = 1),
                     limits = c(-0.05, 0.5)) +
  labs(x = NULL,  # Remove x-axis label to save space
       y = "Risk Reduction with Surgery") +
  ...
  theme(
    ...
    # Angle x-axis text if needed for space
...
# Save with descriptive path
ggsave(
  filename = here::here("figures", "rescue_multiverse_results.pdf"),
  plot = final_plot,
  width = 10,
  height = 5,
  units = "in",
...
ggsave(
  filename = here::here("figures", "rescue_multiverse_results.svg"),
  plot = final_plot,
  width = 10,
  height = 5,
  units = "in"
